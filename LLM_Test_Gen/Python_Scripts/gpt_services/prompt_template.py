instruction = """
    ...
    other methods of the same name across different packages. Providing the FQN helps the LLM correctly reference the focal method within the 
    project structure. The format of FQN is packageName.ClassName.MethodName(ParameterTypes).
    ...
    - Jimple Code Representation: an intermediate representation (IR) of Java bytecode
    ...
    3. The test suite must cover:
        ...
        - Error cases (e.g., null inputs if applicable, conditions leading to exceptions like IllegalArgumentException as shown in Jimple, once again, these are examples only,
        it depends on the method logic itself
        ).
